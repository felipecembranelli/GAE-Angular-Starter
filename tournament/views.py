...
def index(request):
  ...
  return respond(request, user, 'index', {'next': '/', 'courses': courses,
                  'apps': apps, 'tournamentHeats': tournamentHeats})
...
def view_heat_result(request, id = None):
    ...
    return respond(request, None, 'heatresult', 
                   {'heatID': id, 'matchResults': matchResults,
                    'heatResult': heatResult, 'appNames': appNames})
...
def check_app(request, id=None):
    ...
    #Create dictionary and render to template
    return respond(request, None, 'check_app', 
                   {'app': app,'game_status_result': game_status_result,
                    'get_next_move_result': get_next_move_result})
...
def fetch_from_url(appurl, function, jsonRequestDict):
    ...
    try:
        ...
        logging.info('url %s returned json %s in %s microseconds', 
                     url,jsonresponse, microseconds)
...
def play_game(playerX, playerO, referee, tournamentHeat):
        ...
        for i in range(9):
            ...
            if not isMoveValid:
                game.log_move_and_save(move = i,turn = turn,
                                       board = 'Not Valid->' +
                                       move,status=otherPlayer + '  WON')
                return otherPlayer + ' WON'
...
#                  params={'id': int(id)})
    ...
    return http.HttpResponse("Enqueued tournament heat " + str(id) +
...
def view_game_result(request, tournamentHeatID = None,
                     appXID = None, appOID = None):
    ...
    if not game:
        ...
        return http.HttpResponseNotFound('No game found for th ' +
                                         tournamentHeatID + ' appxid ' + appXID +
                                         ' appoid ' + appOID)
